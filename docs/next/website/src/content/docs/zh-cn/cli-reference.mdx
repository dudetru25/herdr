...
检出目录移动后，使用 `workspace retarget` 将工作区指向调用方提供的路径。Herdr 不会搜索检出目录。如果仓库有链接工作树，请先重定向主检出目录，再重定向每个链接检出目录。Herdr 会修复两个工作树指针；如果不能修复，则返回指明损坏指针的类型化错误。
...
`machine list` 输出当前的 `[[machines]]` 注册表。`machine add` 保存一台机器而不打开工作区。`machine import` 保存从 SSH 配置发现的主机别名,`--all` 会导入所有发现的别名。每台机器都使用别名作为名称和目标,并让 `cwd` 保持未设置。`workspace create --machine NAME` 创建基于 SSH 的工作区；`--machine` 与 `--cwd` 不能同时使用。
