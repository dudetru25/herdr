...
`shell_mode = "auto"` 会在 macOS 上启动登录 shell，让 `/usr/libexec/path_helper` 等仅登录时生效的 PATH 设置和 Homebrew shell 初始化在新窗格中运行。在其他平台上，它会保持现有的非登录 shell 行为。使用 `"login"` 可在 Unix 上强制启动登录 shell，使用 `"non_login"` 可禁用登录 shell。Windows shell 没有通用的可移植登录 shell 约定，因此在任一显式模式下都会正常启动已配置的 shell。命令窗格、分离式自定义命令按键绑定和显式 argv 启动仍使用现有的命令执行路径。
...
可通过 `[[machines]]` 注册普通 SSH 目标。指定 `cwd` 时，Herdr 会在远端使用 `cd` 和 `$SHELL`，因此远端登录 shell 必须兼容 POSIX。对于 PowerShell、`cmd.exe` 或其他非 POSIX SSH 目标，请省略 `cwd`。

左键点击 New 会立即创建 Local 工作区。右键点击会打开目标选择器,其中包含 Local、已配置的机器和 Add remote machine。添加表单中的 Import SSH hosts 会读取 `~/.ssh/config` 的别名。可用 Space 或鼠标选择多个别名,然后一次确认并保存。已配置的别名不能选择,界面会显示每个别名的结果。按 Esc 或 Back 返回手动表单时,已输入的值会保留。
