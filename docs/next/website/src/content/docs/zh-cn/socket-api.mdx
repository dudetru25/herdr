...
| 会话 | `session.snapshot` |
| 机器 | `machine.list`、`machine.ssh_hosts`、`machine.add`、`machine.import` |
| 工作区 | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.retarget`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.close` |
...
`workspace.retarget` 会把现有工作区指向调用方提供的绝对 Git 检出路径。Herdr 不会搜索替代路径。它会在新的工作目录中重新启动活动窗格。先重定向主检出目录后,它还会修复链接 worktree 的指针。保存的检出路径不可用时,`workspace.get`、`workspace.list` 和 `session.snapshot` 返回的工作区记录会设置 `stale: true` 并包含 `stale_path`。
...
`machine.list` 返回当前的机器注册表。`machine.ssh_hosts` 从用户的 SSH 配置中发现可导入的别名。`machine.add` 保存一台机器。`machine.import` 按顺序保存多个已发现的别名,并返回每个别名的结果。
