...
The file tree follows the selected workspace. It shows an unavailable state when the checkout cannot be read, and its edge control collapses or restores the panel without changing pane identity.

## Tab

A tab is a layout inside a workspace. Use tabs to separate views like `agents`, `logs`, `server`, or `review`. Unnamed tabs show their current position and type, such as `1 terminal` or `2 file-viewer`.

Left-click `+` to create a terminal tab immediately. Right-click `+`, or open **New tab** from a tab context menu, to choose Terminal or an enabled plugin pane. Use the tab context menu to set a custom name.
