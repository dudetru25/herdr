...
| Machine | `machine.list`, `machine.ssh_hosts`, `machine.add`, `machine.import` |
| Workspace | `workspace.create`, `workspace.list`, `workspace.get`, `workspace.focus`, `workspace.rename`, `workspace.retarget`, `workspace.move`, `workspace.move_block`, `workspace.report_metadata`, `workspace.close` |
...
`workspace.retarget` points an existing workspace at a caller-supplied absolute Git checkout path. Herdr does not search for a replacement. It restarts live panes at the new working directory and repairs linked-worktree pointers when the main checkout is retargeted first. Workspace records returned by `workspace.get`, `workspace.list`, and `session.snapshot` set `stale: true` and include `stale_path` when the saved checkout path is unavailable.
...
`machine.list` returns the live configured registry as `name`, `target`, and optional `cwd` records. `machine.ssh_hosts` lists the aliases discovered from the user's SSH config. `machine.add` accepts `name`, `target`, and optional `cwd`, persists one new entry to `config.toml`, reloads the live registry, and returns the added machine:

```json
{"id":"req_machine_list","method":"machine.list","params":{}}
{"id":"req_machine_add","method":"machine.add","params":{"name":"build","target":"builder@example.com","cwd":"~/src/herdr"}}
{"id":"req_machine_add","result":{"type":"machine_added","machine":{"name":"build","target":"builder@example.com","cwd":"~/src/herdr"}}}
{"id":"req_machine_workspace","method":"workspace.create","params":{"machine":"build","focus":true}}
```

`machine.add` trims the submitted fields. It rejects an empty name or target, a target beginning with `-`, a blank provided `cwd`, or a duplicate machine name. These failures return `invalid_machine` or `machine_already_exists`; config persistence and reload failures return `config_update_failed` or `config_reload_failed`. Every error leaves the live registry unchanged, and persistence failures do not replace the existing config. If the new machine applies while an unrelated config section reports a diagnostic, the method returns success and leaves that diagnostic visible in Herdr. The method does not probe SSH connectivity and does not create a workspace; call `workspace.create` with the returned machine name when you want to open one.

`machine.import` accepts an `aliases` array. It validates every alias, reports
`added`, `already_exists`, or `failed` with a reason for each alias, and writes
all valid new machines in one atomic config update before reloading the live
registry. Imported machines use the alias as both `name` and `target`, with no
`cwd`. An undiscovered alias is reported as `failed` without preventing other
aliases from being imported:

```json
{"id":"req_machine_import","method":"machine.import","params":{"aliases":["build","deploy"]}}
{"id":"req_machine_import","result":{"type":"machine_imported","outcomes":[{"status":"added","alias":"build"},{"status":"already_exists","alias":"deploy"}]}}
```

For `workspace.create`, `machine` and `cwd` are mutually exclusive. Workspace records persist and expose the optional machine registry name; the SSH target is resolved from live config on every future pane launch.
