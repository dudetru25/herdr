...
`machine.list` returns the live configured registry as `name`, `target`, and optional `cwd` records. `machine.add` accepts `name`, `target`, and optional `cwd`, persists one new entry to `config.toml`, reloads the live registry, and returns the added machine:
