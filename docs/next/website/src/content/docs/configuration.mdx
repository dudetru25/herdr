...
`shell_mode = "auto"` starts login shells on macOS so login-only PATH setup such as `/usr/libexec/path_helper` and Homebrew shell initialization runs in new panes. On other platforms, it keeps the existing non-login shell behavior. Use `"login"` to force login-shell startup on Unix, or `"non_login"` to opt out. Windows shells do not share a portable login-shell convention, so Windows keeps the configured shell and starts it normally for either explicit mode. Command panes, detached custom command keybindings, and explicit argv launches keep their existing command execution paths.
...
Register machines that Herdr can open through plain SSH. The remote machine does not need Herdr installed.

Left-click New to create a Local workspace immediately. Right-click New to open a target picker with Local, every configured machine, and Add remote machine. The add form asks for a unique name, an SSH target, and an optional remote working directory. Save & open writes the machine to `config.toml`, reloads the live registry, and opens its workspace. Herdr validates the fields before saving, but it does not probe SSH connectivity; authentication and connection errors appear in the new terminal.

Select Import SSH hosts in the add form to load aliases from `~/.ssh/config`. Use Space or a mouse click to select more than one alias, then confirm once. Herdr marks aliases that are already configured and does not let you select them. It shows the result for each selected alias after the import. Esc or Back returns to the manual form without clearing its fields.
...
[[machines]]
name = "prod"
target = "ssh://deploy.example.com:2222"
```

`name` is the unique, non-empty registry name shown in the picker. `target` must be non-empty, must not begin with `-`, and is passed to `ssh` as one destination argument; it may be an SSH alias, `user@host`, or `ssh://host:port`. `cwd` is optional; when present, it must be non-empty and Herdr starts the remote login there with `cd` and `$SHELL`, so it requires a POSIX-compatible remote login shell. Omit `cwd` for PowerShell, `cmd.exe`, or other non-POSIX SSH targets. The add form trims surrounding whitespace and treats a blank optional working directory as omitted. Invalid fields, duplicate names, and config write or reload failures keep the form open and do not open a workspace.

Use the picker, `herdr workspace create --machine build`, or `workspace.create` in the socket API to create a machine workspace. Every ordinary pane created in that workspace starts a new SSH session. Machine workspaces use the local home directory only as internal identity state and do not participate in Git, worktree, or parent-space discovery.

`herdr server reload-config` replaces the live machine registry for future pane launches. Existing SSH panes are unchanged. Removing or renaming a machine causes subsequent pane, tab, and restore launches bound to its old name to fail instead of falling back to a local shell.

## Approved worker placements

Worker placements are an explicit approval registry for worker runs. Each entry binds a stable target tag to a placement kind, working directory, authorized harnesses, and an approval record. The registry is intentionally an open-ended array, so the individual entries are documented here instead of as flat rows in the [Config reference](/docs/config-reference/).

```toml
[[worker_placements]]
target_tag = "herdr-target:local-macos-primary"
kind = "local_pane"
cwd = "/Users/you/src/herdr"
...
[[worker_placements]]
target_tag = "herdr-target:remote-build"
kind = "remote"
machine = "build"
cwd = "/home/you/src/herdr"
harnesses = ["codex"]

[worker_placements.approval]
reference = "TASK-10:remote-build"
approved_at = "2026-08-02"
```

`target_tag` must be unique and non-empty. `kind` may be `local_pane`, `remote`, `remote_temporary`, or `temporary_subspace`. Remote kinds require a configured `machine`; all entries require a non-empty `cwd`, at least one of the supported harnesses (`codex` or `claude`), and a non-empty approval `reference` and `approved_at` value. Herdr validates the registry before using it and rejects duplicate targets, unsupported harnesses, or mismatched local and remote fields.
