...
`shell_mode = "auto"` は macOS でログインシェルを起動するため、`/usr/libexec/path_helper` のようなログイン時のみの PATH 設定や Homebrew のシェル初期化が新しいペインで実行されます。他のプラットフォームでは、既存の非ログインシェルの挙動を保ちます。Unix でログインシェルの起動を強制するには `"login"` を、無効にするには `"non_login"` を使ってください。Windows のシェルには移植可能な共通ログインシェル規約がないため、どちらの明示モードでも設定済みのシェルを通常どおり起動します。コマンドペイン、デタッチされたカスタムコマンドキーバインド、明示的な argv 起動は、既存のコマンド実行経路を保ちます。
...
`[[machines]]` で通常の SSH 接続先を登録できます。`cwd` を指定すると Herdr はリモート側で `cd` と `$SHELL` を使うため、POSIX 互換のログインシェルが必要です。PowerShell、`cmd.exe`、またはその他の非 POSIX SSH 接続先では `cwd` を省略してください。

New を左クリックすると Local ワークスペースがすぐに作成されます。右クリックすると、Local、設定済みマシン、Add remote machine を含む接続先ピッカーが開きます。追加フォームの Import SSH hosts は `~/.ssh/config` のエイリアスを読み込みます。Space またはマウスで複数選択し、一度の確認で保存できます。設定済みのエイリアスは選択できず、各エイリアスの結果が表示されます。Esc または Back で手動フォームへ戻っても、入力済みの値は保持されます。
