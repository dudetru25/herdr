...
チェックアウトを移動した後は、呼び出し元が指定したパスに `workspace retarget` で変更します。Herdr はチェックアウトを検索しません。リンクされたワークツリーがある場合は、最初にメインチェックアウトを変更し、次に各リンクチェックアウトを変更します。Herdr は両方のワークツリーポインターを修復するか、壊れたポインターを示す型付きエラーを返します。
...
`machine list` は現在の `[[machines]]` レジストリを表示します。`machine add` はワークスペースを開かずに 1 台のマシンを保存します。`machine import` は SSH 設定から検出したホストエイリアスを保存し、`--all` は検出されたすべてのエイリアスを読み込みます。各マシンはエイリアスを名前と接続先に使い、`cwd` は未設定のままです。`workspace create --machine NAME` は SSH ベースのワークスペースを作成します。`--machine` と `--cwd` は同時に指定できません。
