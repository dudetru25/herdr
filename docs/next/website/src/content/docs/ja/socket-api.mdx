...
| マシン | `machine.list`、`machine.ssh_hosts`、`machine.add`、`machine.import` |
| ワークスペース | `workspace.create`、`workspace.list`、`workspace.get`、`workspace.focus`、`workspace.rename`、`workspace.retarget`、`workspace.move`、`workspace.move_block`、`workspace.report_metadata`、`workspace.close` |
...
`workspace.retarget` は、既存のワークスペースを呼び出し元が指定した絶対パスの Git チェックアウトへ変更します。Herdr は代替パスを検索しません。ライブペインは新しい作業ディレクトリで再起動します。メインチェックアウトを先に変更すると、リンクされた worktree のポインターも修復されます。保存済みのチェックアウトパスを利用できない場合、`workspace.get`、`workspace.list`、`session.snapshot` のワークスペースレコードは `stale: true` と `stale_path` を返します。
...
`machine.list` は現在のマシンレジストリを返します。`machine.ssh_hosts` はユーザーの SSH 設定からインポート可能なエイリアスを検出します。`machine.add` は 1 台のマシンを保存し、`machine.import` は複数の検出済みエイリアスを順序どおりに保存して、エイリアスごとの結果を返します。
