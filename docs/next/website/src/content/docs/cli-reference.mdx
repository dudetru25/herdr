...
Use `herdr status --json` for the exact client binary path, client and server
protocol versions, and their compatibility result. The narrower
`status client --json` and `status server --json` forms report only their named
side. From a source checkout, `just reload-local-check` performs the full
read-only compatibility check and also verifies that bare `herdr` resolves to
the repository reload destination. It does not build, install, change `PATH`,
or hand off the server.
...
herdr workspace create [--cwd PATH | --machine NAME] [--label TEXT] [--env KEY=VALUE] [--focus] [--no-focus]
herdr workspace get <workspace_id>
herdr workspace focus <workspace_id>
herdr workspace rename <workspace_id> <label>
herdr workspace retarget <workspace_id> <path>
...
Use `workspace retarget` after a checkout moves to a caller-supplied path. Herdr does not search for the checkout. For a repository with linked worktrees, retarget the main checkout first and then each linked checkout. Herdr repairs both linked-worktree pointers or returns a typed error that names the broken pointer.

## Machines

```bash
herdr machine list
herdr machine add --name NAME --target TARGET [--cwd PATH]
herdr machine import <ALIAS>...
herdr machine import --all
herdr machine ssh-hosts [--json]
herdr workspace create --machine build
```

`machine list` prints the live `[[machines]]` registry. `machine add` saves one
machine without opening a workspace. `machine import` saves discovered SSH
host aliases as machines; `--all` imports every alias returned by the user's
SSH config. Each imported machine uses the alias as both its name and target
and leaves `cwd` unset. `workspace create --machine NAME` creates an SSH-backed
workspace; `--machine` and `--cwd` are mutually exclusive.
