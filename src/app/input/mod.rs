        ...
        if !handled_pane_double_click {
            if let Some(action) = self.state.handle_mouse(&mut self.terminal_runtimes, mouse) {
                match action {
                    ...
                    MouseAction::AddRemoteMachineImportOpen => self.open_machine_import(),
                    MouseAction::AddRemoteMachineImportSubmit => self.submit_machine_import(),
...
pub(crate) fn modal_paste_target_active(state: &AppState) -> bool {
    match state.mode {
        ...
        Mode::AddRemoteMachine => state.machine_create.as_ref().is_none_or(|create| {
            create.import.is_none()
                && create.focused != crate::app::state::MachineCreateField::Import
        }),
