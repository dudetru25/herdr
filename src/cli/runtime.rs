use crate::api::schema::{
    ...
    WorkspaceRetargetParams, WorkspaceTarget, WorktreeCreateParams, WorktreeListParams,
...
pub(super) fn workspace_retarget(params: WorkspaceRetargetParams) -> std::io::Result<i32> {
    print_method_response("cli:workspace:retarget", Method::WorkspaceRetarget(params))
}

pub(super) fn workspace_become_parent(params: WorkspaceParentSpaceParams) -> std::io::Result<i32> {
    print_method_response(
        "cli:workspace:become-parent",
        Method::WorkspaceBecomeParent(params),
    )
}

pub(super) fn workspace_rescan_children(
    params: WorkspaceParentSpaceParams,
) -> std::io::Result<i32> {
    print_method_response(
        "cli:workspace:rescan-children",
        Method::WorkspaceRescanChildren(params),
    )
}

pub(super) fn workspace_stop_parent(params: WorkspaceParentSpaceParams) -> std::io::Result<i32> {
    print_method_response(
        "cli:workspace:stop-parent",
